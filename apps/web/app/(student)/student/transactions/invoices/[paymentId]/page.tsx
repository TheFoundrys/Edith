...
import { loadStudentInvoicePayment } from "@/lib/payments/invoice-queries";
...
export default async function StudentInvoicePage({
  ...
  const payment = await loadStudentInvoicePayment(
    paymentId,
    session.user.id,
  ...
  const taxable = Math.max(0, principal - discount);
    ...
    <div>
      ...
      <Panel className="p-6 sm:p-8 max-w-3xl">
        ...
        <div className="mt-8 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tfoot>
              ...
              {gst > 0 ? (
                <tr>
                  <td colSpan={2} className="pt-2 text-xs text-fg-muted text-right">
                    Taxable value {formatCurrency(taxable, payment.currency)} + GST{" "}
                    {formatCurrency(gst, payment.currency)}
                  </td>
                </tr>
              ) : null}
        ...
        <p className="mt-8 text-xs text-fg-muted leading-relaxed">
          This is a computer-generated invoice from {APP_NAME}. For billing support, open a
          help ticket from your student account.
        </p>
      </Panel>
    </div>
  );
